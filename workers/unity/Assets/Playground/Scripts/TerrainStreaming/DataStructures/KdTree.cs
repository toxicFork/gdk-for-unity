...
public class KdTree<T> : IEnumerable<T>, IEnumerable where T : Component
{
    ...
    protected float _distanceSquared(Vector3 a, Vector3 b)
    {
        if (_just2D)
    ...
    /// <param name="position">position</param>
    /// <returns>closest object</returns>
    public T FindClosestExceptSelf(Vector3 position, T self)
    {
        return _findClosest(position, self: self);
    }
    ...
    protected T _findClosest(Vector3 position, List<T> traversed = null, T self = null)
    {
        ...
        while (openCur < _open.Length && _open[openCur] != null)
        {
            ...
            var nodeDist = _distanceSquared(position, current.component.transform.position);
            if (nodeDist < nearestDist && current.component != self)
            {
    ...
    /// <summary>
    /// Find object in Range to given position
    /// </summary>
    /// <param name="position">position</param>
    /// <param name="radius">radius</param>
    /// <param name="matches">matches</param>
    public void FindRange(Vector3 position, float radius, ref List<T> matches)
    {
        if (matches == null)
            matches = new List<T>();
        matches.Clear();
        ...
        while (openCur < _open.Length && _open[openCur] != null)
        {
            ...
            if (_distanceSquared(position, current.component.transform.position) <= radius * radius)
                matches.Add(current.component);

            var splitCurrent = _getSplitValue(current);
            var splitSearch = _getSplitValue(current.level, position);

            if (splitSearch - radius < splitCurrent)
            {
                if (current.left != null)
                    _open[openAdd++] = current.left; //go left
            }
            if (splitCurrent < splitSearch + radius)
            {
                if (current.right != null)
                    _open[openAdd++] = current.right; //go right
            }
